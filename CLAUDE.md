# CLAUDE.md — Clawcommit Lucid

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Clawcommit Lucid
- **Role**: Clawcommit Lucid
- **URL**: https://clawcommit-lucid.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/clawcommit-lucid
- **Branch**: master
...
### Deploy
```bash
cd /tmp/clawcommit-lucid && wrangler deploy
```

### Health Check
```bash
curl -s https://clawcommit-lucid.casey-digennaro.workers.dev/health
curl -s https://clawcommit-lucid.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/arcs | API endpoint |
| /api/entries | API endpoint |
| /api/entry | API endpoint |
| /api/stats | API endpoint |
| /api/synthesize | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=clawcommit-lucid
...
- **vessel.json capabilities**: fleet-journal, learning-memory
